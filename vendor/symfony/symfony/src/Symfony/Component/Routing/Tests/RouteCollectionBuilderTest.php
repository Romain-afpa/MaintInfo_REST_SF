...
class RouteCollectionBuilderTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testAdd()
    {
        ...
        $addedRoute = $collectionBuilder->add('/checkout', 'ServiceBundle:Order:checkout');
        $addedRoute2 = $collectionBuilder->add('/blogs', 'ServiceBundle:Blog:list', 'blog_list');
        $this->assertInstanceOf('Symfony\Component\Routing\Route', $addedRoute);
        $this->assertEquals('ServiceBundle:Order:checkout', $addedRoute->getDefault('_controller'));
    ...
    public function testFlushOrdering()
    {
        ...
        // 1) Add a route
        $routes->add('/checkout', 'ServiceBundle:Order:checkout', 'checkout_route');
        // 2) Import from a file
        $routes->mount('/', $routes->import('admin_routing.yml'));
        // 3) Add another route
        $routes->add('/', 'ServiceBundle:Default:homepage', 'homepage');
        // 4) Add another route
        $routes->add('/admin', 'ServiceBundle:Admin:dashboard', 'admin_dashboard');
    ...
    public function testFlushSetsRouteNames()
    {
        ...
        // add a "named" route
        $collectionBuilder->add('/admin', 'ServiceBundle:Admin:dashboard', 'admin_dashboard');
        // add an unnamed route
        $collectionBuilder->add('/blogs', 'ServiceBundle:Blog:list')
            ->setMethods(array('GET'));

        // integer route names are allowed - they don't confuse things
        $collectionBuilder->add('/products', 'ServiceBundle:Product:list', 100);
